...
module TestGroupdate
  ...
  def assert_date(function, expected, time_str)
    ...
    assert_sql expected, "SELECT g#{function}('#{time_str}'::date)"
    assert_sql expected, "SELECT g#{function}('#{time_str}'::date, 'America/Los_Angeles')"
  ...
  def assert_time(function, expected, time_str)
    expected = Date.parse(expected)
    assert_sql expected, "SELECT g#{function}('#{time_str}'::timestamp)"
    assert_sql expected, "SELECT g#{function}('#{time_str}'::timestamptz)"
    assert_sql expected, "SELECT g#{function}('#{time_str}'::timestamp, 'America/Los_Angeles')"
    assert_sql expected, "SELECT g#{function}('#{time_str}'::timestamptz, 'America/Los_Angeles')"
  ...
  def assert_time_utc(function, expected, time_str)
    expected = Date.parse(expected)
    assert_sql expected, "SELECT g#{function}('#{time_str}'::timestamp, 'Etc/UTC')"
    assert_sql expected, "SELECT g#{function}('#{time_str}'::timestamptz, 'Etc/UTC')"
